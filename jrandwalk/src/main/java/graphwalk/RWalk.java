        ...
        for (Node x : neighbors.neighbors) {
            ...
            if (t != null) {
                ...
                if (rWalkMode.equals("Node2vec")) {
                    if (x.id == t.id) {
                        graph_case = 0;
                    } else if (edgeMap.get(tx) == null && edgeMap.get(xt) == null) {
                        graph_case = 1;
                    } else if (edgeMap.get(tx) != null || edgeMap.get(xt) != null) {
                        graph_case = 2;
                    }
...
public class RWalk {
    ...
    void constructAndSaveWalks() throws IOException {
        ...
        for (int node_id: g.nodePostings.keySet()) {
            ...
            for (int i = 0; i < numWalks; i++) {
                bw.write(constructAndSaveWalkFromRandomSrc(t));
                bw.newLine();
            }
        }
        bw.close();
    ...
    String constructAndSaveWalkFromRandomSrc(Node t) {
        buff.setLength(0);
        List<Integer> nodesequence = walk(t, numSteps, alpha, beta, 0);
